    ...
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 gap-6">
        {displayError && appState !== 'analyzing' && (
          <div className="bg-danger/10 border border-danger text-danger rounded-xl px-4 py-3 max-w-md w-full text-center text-sm">
            {displayError}
          </div>
        )}

        <div className={`relative w-full max-w-md ${showResult || showError ? 'hidden' : ''}`}>
          <CameraView
            videoRef={videoRef}
            canvasRef={canvasRef}
